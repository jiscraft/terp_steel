/**
 * Created by jiscraft on 2016-03-04.
 */
Ext.define('Terp.view.tsoft.componentux.plugin.TsoftTreeFilterPlugin', {
    ...
    filter: function (value, property, re) {
        ...
        tree.getStore().getFilters().add(function(item) {
            var invalid = (item.get('text').toUpperCase() === 'ROOT') || Ext.isEmpty(item.get(property)) || (item.get(property).toUpperCase() === 'ROOT');
            if (!invalid) {
                console.log(value, item.get(property), item.isLeaf(), item.get(property).indexOf(value), (item.isLeaf() && item.get(property).indexOf(value) > -1), item);
            }
            return (!invalid && item.isLeaf() && (item.get(property).indexOf(value) > -1));
        });
        tree.expandAll();

        /* 사용안함 - 20210803 Andrew
    ...
    clearFilter: function () {
        ...
        tree.getStore().getFilters().removeAll();
        tree.collapseAll();
