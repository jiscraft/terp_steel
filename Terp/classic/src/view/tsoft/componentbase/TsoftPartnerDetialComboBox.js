 ...
 */
Ext.define('Terp.view.tsoft.componentbase.TsoftPartnerDetialComboBox', {
    extend: 'Ext.form.field.ComboBox',
    xtype: 'tsoftpartnerdetial',

    typeOfPartner: 'grid',
    dataIndexOfPartner: 'cd_p',
    textOfPartner: '거래처',
    ...
    onFocus: function(fld){
        ...
        var val = null;
        if (this.typeOfPartner === 'grid') {
            val = this.ownerCt.column.ownerCt.grid.getSelectionModel().getLastSelected().get(this.dataIndexOfPartner);
        }
        else if (this.typeOfPartner === 'form') {
            val = this.ownerCt.up('form').getForm().findField(this.dataIndexOfPartner).getValue();
        }
        if (Ext.isEmpty(val)) {
            Terp.app.getController('TerpCommon').toastMessage(this.textOfPartner + '을 먼저 선택해야 합니다','b');
            return false;
        }
